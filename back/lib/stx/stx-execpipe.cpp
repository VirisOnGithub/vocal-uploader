...
class RingBuffer {
  ...
  public:
	...
	inline RingBuffer() : m_data(NULL), m_buffsize(0), m_size(0), m_bottom(0) {}
	...
	inline unsigned int bottomsize() const {
		return (m_bottom + m_size > m_buffsize) ? (m_buffsize - m_bottom) : (m_size);
	}
	...
	void write(const void* src, unsigned int len) {
		...
		if (m_buffsize < m_size + len) {
			...
			if (m_bottom + m_size > m_buffsize) {
				...
				memcpy(m_data + newbuffsize - taillen, m_data + m_bottom, taillen);
		...
		if (m_bottom + m_size > m_buffsize) {
			...
			if (tailfit >= len) {
				...
				memcpy(m_data, reinterpret_cast<const char*>(src) + tailfit, len - tailfit);
				m_size += len;
			}
		}
	}
...
class ExecPipeImpl {
  ...
  private:
	...
	struct Stage {
		...
		Stage() :
			prog(NULL), argsp(NULL), envp(NULL), func(NULL), withpath(false), pid(0), retstatus(0), stdin_fd(-1), stdout_fd(-1) {}
	};
...
void ExecPipeImpl::run() {
	...
	for (stagelist_type::const_iterator st = m_stages.begin(); st != m_stages.end(); ++st) {
		if (st->func)
			continue;
	...
	while (1) {
		...
		for (unsigned int i = 0; i < m_stages.size(); ++i) {
			...
			if (m_stages[i].stdin_fd >= 0) {
				...
				if (max_fds < m_stages[i].stdin_fd)
					max_fds = m_stages[i].stdin_fd;
			...
			if (m_stages[i].stdout_fd >= 0) {
				if (m_stages[i].outbuffer.size()) {
					...
					if (max_fds < m_stages[i].stdout_fd)
						max_fds = m_stages[i].stdout_fd;
		...
		if (m_input_fd >= 0 && FD_ISSET(m_input_fd, &write_fds)) {
			if (m_input == ST_STRING) {
				...
				do {
					try {
						wb = write(
							m_input_fd, m_input_string->data() + m_input_string_pos, m_input_string->size() - m_input_string_pos);
				...
				do {
					wb = write(m_input_fd, m_input_rbuffer.bottom(), m_input_rbuffer.bottomsize());
		...
		if (m_output_fd >= 0 && FD_ISSET(m_output_fd, &read_fds)) {
			...
			do {
				...
				rb = read(m_output_fd, m_buffer, sizeof(m_buffer));
		...
		for (unsigned int i = 0; i < m_stages.size(); ++i) {
			...
			if (m_stages[i].stdin_fd >= 0 && FD_ISSET(m_stages[i].stdin_fd, &read_fds)) {
				...
				do {
					...
					rb = read(m_stages[i].stdin_fd, m_buffer, sizeof(m_buffer));
			...
			if (m_stages[i].stdout_fd >= 0 && FD_ISSET(m_stages[i].stdout_fd, &write_fds)) {
				while (m_stages[i].outbuffer.size() > 0) {
					ssize_t wb = write(m_stages[i].stdout_fd, m_stages[i].outbuffer.bottom(), m_stages[i].outbuffer.bottomsize());
...
ExecPipe::ExecPipe() : m_impl(new ExecPipeImpl) {
	++m_impl->refs();
}
...
ExecPipe::ExecPipe(const ExecPipe& ep) : m_impl(ep.m_impl) {
	++m_impl->refs();
}
...
void ExecPipe::add_exece(const char* path, const std::vector<std::string>* args, const std::vector<std::string>* env) {
...
PipeSource::PipeSource() : m_impl(NULL) {}
...
PipeFunction::PipeFunction() : m_impl(NULL), m_stageid(0) {}
